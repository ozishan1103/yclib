...
class YcSession:
    ...
    def write_dataframe_to_s3(self, dataframe, tar_file_path, data_format):
    ...
    def read_s3_data_into_dataframe(self, pd, src_path, data_format):
    ...
    def write_dataframe_to_s3_ws(self, dataframe, tar_file_path, data_format, args=None):
        session = self.get_boto_session()
        if data_format == "csv":
            wr.s3.to_csv(
                df=dataframe,
            ...
            raise ValueError("Pass a valid data format (csv/json/parquet)")

    def read_s3_data_into_dataframe_ws(self, src_path, data_format, args=None):
        session = self.get_boto_session()
        if data_format == "csv":
